...
spray_sent = False  # Tránh gửi tín hiệu phun nước nhiều lần
normal_duration = 5  # Thời gian tài xế ở trạng thái "normal"
abnormal_duration = 0  # Thời gian ở trạng thái bất thường (drowsy, yawning)
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  
                        ...
                        if names[c] == 'normal':
                            ...
                            # 🔹 Nếu "normal" ≥ 2 giây → Tắt cảnh báo và reset trạng thái phun nước
                            if normal_duration >= 2:
                                if alert_sent:
                                    print("✅ Gửi tín hiệu TẮT cảnh báo đến ESP8266!")  
                                    try:
                                        requests.get(ESP_IP + "/normal", timeout=3)
                                        alert_sent = False
                                    except Exception as e:
                                        print("❌ Lỗi gửi tín hiệu TẮT còi:", e)

                                # Reset trạng thái phun nước
                                spray_sent = False  
                            ...
                            # Giới hạn tối đa thời gian theo dõi trong 15 giây
                            abnormal_duration = min(abnormal_duration, 15)  
                            normal_duration = min(normal_duration, 15)  

                            # 🚨 Nếu **bất thường** ≥ 3 giây → Bật còi cảnh báo
                            if abnormal_duration >= 3 and not alert_sent:
                                print("🚨 Gửi cảnh báo đến ESP8266!")  
                                try:
                                    requests.get(ESP_IP + "/alert", timeout=3)
                                    alert_sent = True
                                except Exception as e:
                                    print("❌ Lỗi gửi tín hiệu BẬT còi:", e)

                            # 💦 Nếu **bất thường** ≥ 15 giây → Kích hoạt phun nước
                            if abnormal_duration >= 15 and not spray_sent:
                                print(f"💦 Gửi lệnh phun nước! (Thời gian bất thường: {abnormal_duration:.2f} giây)")  
                                try:
                                    response = requests.get(ESP_IP + "/spray", timeout=10)
                                    if response.status_code == 200:
                                        spray_sent = True  # Chỉ đánh dấu đã gửi nếu ESP8266 phản hồi thành công
                                except Exception as e:
                                    print("❌ Lỗi gửi tín hiệu phun nước:", e)
                        # 🔄 Cập nhật thời điểm xử lý frame cuối cùng
                        last_update_time = time.time()

                        # 🖌 Gán màu hiển thị cho bounding box
